...
//P4OUT ^= 1 << 6; //toggle red led				// The control is comming out of this while loop so no problem
...
*/
/**************************************************************************/
void LoRaWan::sendData(uint8_t Frame_Port, const char *Data, const uint8_t Data_Length )
{
  //Define variables
  unsigned char i;
...
//  Encrypt_Payload(tmpData, Data_Length, _frameCounter, Direction);
  ...
  //Calculate MIC
  //Calculate_MIC(LORA_Data, MIC, LORA_Package_Length, _frameCounter, Direction); 		//I m not doing AES so this is not required
...
void memcpy(unsigned char *target, unsigned char *source, int n){
